...
module.exports = function () {
    ...
    // Get All Guest Users
    app.get('/api/guestUsers', function(req, res) {
        User.find({
            usergroup: 'Guest'
        }, function (err, users) {
            if (err)
    ...
    // Delete Guest
    app.delete('/api/guestUsers/:User_Id', async function (req, res) {

        try {

            let username = req.params.User_Id.toLowerCase().trim();

            await User.remove({
                'username': username
            }).exec();

            User.removeFileSystem(username);

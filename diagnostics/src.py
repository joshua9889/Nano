#===============================================================================
# diagnostics/src.py - diagnostic subsystem
#-------------------------------------------------------------------------------
# Revision History:
#   20-Dec-2018 <jwa> - Added output of Fusion Runtime Version info from
#						/usr/Fusion/version.txt
#
#===============================================================================
        ...
        with open(in_file, 'rb') as inF:
            for line in inF:
                if '#' not in line:
                    line = line.replace('\n', '')
                    line = line.replace('\r', '')
                    line = line.replace(chr(0x09), '')
                    temp = line.split(';')
                    temp[1] = int(temp[1].replace(' ', ''), 16)
                    temp[2] = int(temp[2].replace(' ', ''), 16)
                    out_array[0].append(temp[0])
                    out_array[1].append(temp[1])
                    out_array[2].append(temp[2])
    except:
...
class FusionDiagnostics(App):
    def __init__(self, *args):
        ...
        super(FusionDiagnostics, self).__init__(*args, static_file_path=res_path, css_head=my_css_head)    

        self.mri_devices = parseConfFile(res_path + '/mri_devices.conf')
...
#--------------------------------------------------------------------------------------------------
        ...
        self.i2c_dev_buttons[0].set_on_click_listener(self.on_i2c_dev_0_click)
        ...
        self.i2c_dev_buttons[1].set_on_click_listener(self.on_i2c_dev_1_click)
        ...
        self.i2c_dev_buttons[2].set_on_click_listener(self.on_i2c_dev_2_click)
        ...
        self.i2c_dev_buttons[3].set_on_click_listener(self.on_i2c_dev_3_click)
...
#--------------------------------------------------------------------------------------------------   
    ...
    def on_stats_pressed(self, widget):
        ...
        # Get Version info 
        text_output += 'Fusion RunTime: '
        process = Popen(['cat', '/usr/Fusion/version.txt'], stdout=PIPE)
        output, _error = process.communicate()
        text_output += str(output) + '\n\n'
    ...
    def on_download_pressed(self, widget):
        try:
            ...
            jmo = self.diagnosticDownload('jmo', 'fullDiagnosticOutput.txt', width=200)
    ...
    def on_refresh_pressed(self, widget):
        for i in range(len(self.i2c_dev_buttons)):
            self.i2c_dev_buttons[i].set_text('Scanning')
            self.i2c_dev_buttons[i].set_enabled(False)
        self.refresh_flag = True
        
    def on_i2c_dev_0_click(self, widget): self.changeAddress(0)        
    def on_i2c_dev_1_click(self, widget): self.changeAddress(1) 
    def on_i2c_dev_2_click(self, widget): self.changeAddress(2)
    def on_i2c_dev_3_click(self, widget): self.changeAddress(3)
    
    def changeAddress(self, index):
        self.change_index = index 
        self.addr_change = gui.GenericDialog('MRI Device I2C address change', 'Enter a new address, must be an even number and between 0x10 and 0xEE', width=500, height=180)
        self.addr_change.add_field('current_label', gui.Label('Current Address = ' + str(self.addresses[index])))
        self.address_entry = gui.TextInput(single_line=True, hint="New Address 0x", width='99%', height=25, style={'line-height': '25px', 'text-align': 'center', 'position': 'relative', 'left': '23px', 'border-color': 'gray', 'border-width': '1px', 'border-style': 'solid'})
        self.addr_change.add_field('address_entry', self.address_entry)
        self.addr_change.set_on_confirm_dialog_listener(self.on_change_confirmed)
        self.addr_change.show(self)
    
    def on_change_confirmed(self, widget):
        try:
            new_addr = int(self.addr_change.get_field('address_entry').get_value(), 16)
            if(((new_addr & 0x01) == 0x01) or (new_addr < 0x10)): pass 
            else:
                f.setI2CAddress(int(self.addresses[self.change_index], 16), new_addr)
            time.sleep(0.5)
            self.refresh_flag = True
        except: 
            print traceback.format_exc(1)
    ...
    def controllerUpdateThread(self):
        ...
        if(self.refresh_flag == True):
            time.sleep(0.2)
            self.refresh_flag = False             
            self.addresses = f.getI2CAddresses()         
            for i in range(len(self.i2c_dev_buttons)):
                self.i2c_dev_buttons[i].set_text('')          
            
            for i in range(len(self.addresses)):
                dev_id = f.i2cRead(int(self.addresses[i], 16), 0x02, 1)[0]                
                if dev_id in self.mri_devices[1]:
                    self.i2c_dev_buttons[i].set_text("%s [%s]" % (self.mri_devices[0][self.mri_devices[1].index(dev_id)], str(self.addresses[i])))
                    print "%s [%s]" % (self.mri_devices[0][self.mri_devices[1].index(dev_id)], str(self.addresses[i]))
                    self.i2c_dev_buttons[i].set_enabled(True)                    
                else:
                    self.i2c_dev_buttons[i].set_text("Unknown [%s]" % (str(self.addresses[i])))
